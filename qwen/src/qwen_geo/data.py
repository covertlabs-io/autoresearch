...
def detect_columns(dataset: Dataset, config: DatasetConfig) -> dict[str, str | None]:
...
def _normalize_split(dataset: Dataset, columns: dict[str, str | None]) -> Dataset:
...
def _load_raw_bundle(config: DatasetConfig) -> DatasetDict:
    kwargs: dict[str, Any] = {}
...
def _split_bundle(bundle: DatasetDict, config: DatasetConfig) -> DatasetDict:
...
def load_prepared_dataset(prepared_path: Path) -> DatasetDict:
    return load_from_disk(str(prepared_path))


def limit_split(dataset: Dataset, limit: int | None) -> Dataset:
